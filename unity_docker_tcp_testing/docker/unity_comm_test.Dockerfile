...
# 2. Make sure only a TestTCPClient is attached to the tcp game object in unity
...
# 1. remove `&& sleep 20 \`, `&& sleep 20 \` lines from run_test.sh
# 2. Set ip of client_test.cpp to your host computer's IPv4 addres (ex. `uint8_t ip[] = {192,168,1,105};` for me)
# 3. Make sure only a TestTCPServer is attached to the tcp game object in unity
